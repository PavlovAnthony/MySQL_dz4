...
SELECT MARK, color, COUNT(color) AS 'num of cars' FROM AUTO
WHERE MARK = 'LADA'
GROUP BY color, mark 
UNION SELECT MARK, color, COUNT(color) AS 'num of cars' FROM AUTO
WHERE MARK = 'BMW'
GROUP BY color, mark;

--  Вывести на экран марку авто и количество AUTO не этой марки
SELECT MARK, (COUNT(color) - 1) AS 'num of cars' FROM AUTO
GROUP BY MARK;
